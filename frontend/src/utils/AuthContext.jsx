  ...
  const checkAuthStatus = async (token) => {
    ...
    try {
      ...
      if (response.status === 200) {
        setIsLoggedIn(true);
        setUserType(response.data.is_landlord ? 'landlord' : 'tenant');  // Adjust role
        console.log("User type:", response.data.is_landlord ? 'landlord' : 'tenant'); 
      } else {
  ...
  const login = async (email, password) => {
    try {
      ...
      if (response.status === 200) {
        ...
        setUserType(response.data.is_landlord ? 'landlord' : 'tenant');  // Determine role from response
        setIsLoggedIn(true);
      } else {
